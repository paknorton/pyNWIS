...
stn_url = '%s/site/?format=rdb&sites=%s&siteOutput=expanded&siteStatus=active&parameterCd=%s&siteType=ST' \
          % (base_url, sitelist, paramList)

logging.info('Water years: %s' % (args.statRepType == 'annual' and args.wateryears))
logging.info('Sites: %s' % sitelist)
